...
export async function save(type, value, btn, customTimestamp = null) {
  ...
  if (type !== "sleep") {
    ...
    const lastSleepEvent = [...allEntries]
      .reverse()
      .find((e) => e.type === "sleep" && !e.deleted);
    const isAsleep =
      lastSleepEvent &&
      (lastSleepEvent.value === "sleeping" || lastSleepEvent.value === "nap");
...
export async function updateDailyReport() {
  ...
  updateRecentEvents(entriesInDay, updateDailyReport, () =>
    updateButtonStates(loadTodayEntries)
  );
